...
export function WeatherPanel({ showId, className }: WeatherPanelProps) {
  ...
  const updateWeatherData = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update weather data: ${response.statusText}`
        );
      }
      const data = await response.json();
      setWeatherData(data);
      ...
      setError(
        err instanceof Error ? err.message : "Failed to update weather data"
      );
    } finally {
      setUpdating(false);
    }
  ...
  return (
    <Card className={className}>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <RefreshCw
              className={cn(
                "h-4 w-4 mr-1",
                (updating || weatherData.isUpdating) && "animate-spin"
              )}
            />
            Update
          </Button>
      ...
      <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-gray-600">
                      {formatTemp(climateProfile.avgHighTempC)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-gray-600">
                      {formatTemp(climateProfile.avgLowTempC)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-gray-600">
                      {Math.round(climateProfile.avgPrecipDays)} days
                    </div>
                  </div>
                </div>
                ...
                {climateProfile.avgWindSpeed && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">
                        {Math.round(climateProfile.avgWindSpeed)} km/h
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <div className="text-xs text-gray-500 space-y-1">
                    {climateProfile.hotDaysPct > 0 && (
                      <div>
                        Very hot days (&gt;30°C):{" "}
                        {Math.round(climateProfile.hotDaysPct)}%
                      </div>
                    )}
                    {climateProfile.coldDaysPct > 0 && (
                      <div>
                        Very cold days (&lt;5°C):{" "}
                        {Math.round(climateProfile.coldDaysPct)}%
                      </div>
                    )}
                  </div>
                </div>
        ...
        {/* Recommendations */}
        {weatherDetail?.recommendations &&
          weatherDetail.recommendations.length > 0 && (
            <div className="bg-blue-50 p-3 rounded-lg">
              <h5 className="font-medium text-sm text-blue-900 mb-2">
                Recommendations
              </h5>
              <ul className="text-sm text-blue-800 space-y-1">
                {weatherDetail.recommendations.map(
        ...
        {climateProfile && (
          <div className="text-xs text-gray-400 pt-2 border-t border-gray-100">
            Data last updated:{" "}
            {new Date(climateProfile.lastUpdated).toLocaleDateString()}
          </div>
        )}
      </CardContent>
    </Card>
